		...
		try {
			...
			for (int index = 1; index < sheet.getRows(); index++) {
				...
				for (int jindex = 0; jindex < sheet.getColumns(); jindex++) {
					if (jindex == 0) {
						if (sheet.getCell(jindex, index).getContents().trim()
								.length() != 0
								&& !sheet.getCell(jindex, index).getContents()
										.trim().equals("[]")) {
					...
					if (jindex == 4) {
						...
						if (sheet.getCell(jindex, index).getContents().trim()
								...
								&& !sheet.getCell(jindex, index).getContents()
										.trim().equals("[]")) {
							workingDepartmentList.add(sheet.getCell(jindex,
									index).getContents().trim());
					...
					if (jindex == 5) {
						...
						if (sheet.getCell(jindex, index).getContents().trim()
								...
								&& !sheet.getCell(jindex, index).getContents()
										.trim().equals("[]")) {
							imContactInformationList.add(sheet.getCell(jindex,
									index).getContents().trim());
							pc
									.setImContactInformationList(imContactInformationList);
						}
					}
